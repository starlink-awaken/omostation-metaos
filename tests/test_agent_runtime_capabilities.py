...
from metaos.integrations.agent_runtime.mcp_policy import requested_mcp_tools
...
def test_mcp_tool_request_policy_preserves_server_wildcard_and_specific_tool() -> None:
    session = _session(
        risk=OperationalRisk.R3,
        mode=ExecutionMode.COMMIT,
        profile="external-commit",
        requested=["mcp:calendar:create_event", "mcp:calendar:create_event", "mcp:docs"],
    )

    assert validate_capability_profile(session) == []
    assert requested_mcp_servers(session) == ("calendar", "docs")
    assert requested_mcp_tools(session.capability.requested) == {
        "calendar": ("create_event",),
        "docs": ("*",),
    }


def test_invalid_mcp_tool_request_is_rejected() -> None:
    session = _session(
        risk=OperationalRisk.R1,
        mode=ExecutionMode.OBSERVE,
        profile="research-read",
        requested=["mcp:calendar:"],
    )

    assert "Invalid MCP request" in validate_capability_profile(session)[0]
...
def test_provider_context_carries_resolved_policy_without_granting_unrequested_mcp() -> None:
    ...
    assert context.capability_policy["allowed_mcp_tools"] == {"web-reader": ["open"]}
    assert context.environment["METAOS_ALLOWED_MCP_JSON"] == '["web-reader"]'
    assert context.environment["METAOS_ALLOWED_MCP_TOOLS_JSON"] == '{"web-reader": ["open"]}'
