...
def test_local_init_keeps_metaos_out_of_shared_gitignore(tmp_path: Path) -> None:
    ...
    assert (project / ".metaos" / "tasks").is_dir()
...
def test_task_envelope_is_machine_readable(tmp_path: Path) -> None:
...
def test_status_reports_managed_state(tmp_path: Path) -> None:
    home = tmp_path / "home"
    project = tmp_path / "project"
    install_global(home=home, providers=("codex",), apply=True)
    ...
    assert result["project_metaos_exists"] is True
    assert result["codex_global_marker"] is True


def test_launch_options_are_not_forwarded_to_provider(tmp_path: Path, capsys) -> None:
    ...
    assert payload["command"] == ["codex", "--help"]
    assert payload["env"]["METAOS_MODE"] == "stage"
    assert payload["env"]["METAOS_PROJECT_ROOT"] == str(project.resolve())
